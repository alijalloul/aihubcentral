...
const Context = () => {
  ...
  const handleContextualizedQuestion = async (e) => {
    try {
      ...
      const res = await fetch(`${BASE_URL}/api/openai/context`, {
        method: "POST",
        headers: {
    ...
    <LazyMotion features={domAnimation}> 
      ...
      <div className='w-[40%] h-full mt-0 flex flex-col items-center mb-10 md:w-[80%]'>
        <div className='w-full'>
          <div style={{textOverflow: "word-break"}} className=' text-6xl mb-5 leading-[5rem] md:text-5xl'> 
        ...
        <div className='w-full h-fit h-max-[40%] mt-3'>
          <div style={{scrollbarWidth: "thin"}} className='w-full h-full px-5 py-5 flex flex-col border border-3 border-black rounded-md overflow-y-scroll'>
            {
              loading ? (
                <div>
