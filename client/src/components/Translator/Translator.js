    ...
    <LazyMotion features={domAnimation}> 
      <m.div  
        ...
        <div className='w-[50%] mt-5 mb-10 text-6xl leading-[5rem] md:w-[80%] md:text-5xl md:mb-5'>
            <span className='inline-block bg-[rgb(255,50,0)] bg-[linear-gradient(45deg,rgba(255,50,0,1)_0%,rgba(255,132,0,1)_100%)] bg-clip-text text-transparent font-bold'>Translate &nbsp;</span> 
        ...
        <div className='w-[80%] h-[50vh] flex justify-center items-center md:flex-col md:h-[95vh]'>
...

            {
                (selectLanguageFrom === true) ? (
                    <div className='h-full'>
                        <input ref={excludedDivRef1} className='w-full h-[10%] text-center md:h-[20%]' placeholder='Search' onChange={ handleSearchLanguageFrom }></input>
                        <div className='overflow-y-scroll h-[80%]'>
                            {
                                languageFromOptions.map((language, index) => (
                                    <button key={index} onClick={ (e) => handleLanguageFrom(e,language) } className='w-full h-[15%]  md:h-[33%]'>{`${language.language}/${language.nativeWriting}`}</button>
                                ))
                            }
                        </div>  
                    </div>
            ...
            <button onClick={ handleTranslate } className='mr-5 px-5 py-5 bg-[rgb(255,50,0)] bg-[linear-gradient(45deg,rgba(255,50,0,1)_0%,rgba(255,132,0,1)_100%)] text-white text-xl font-bold tracking-widest rounded-xl md:mb-5 md:py-2 md:mr-0'>Translte</button>
            ...
            <div className='w-[40%] h-full flex flex-col border-2 border-gray-150 shadow-md shadow-gray-500 rounded-lg overflow-hidden md:w-full md:h-[30%]'>
            {
                (selectLanguageTo === true) ? (
                    <div className='h-full'>
                        <input ref={excludedDivRef2} className='w-full h-[10%] text-center md:h-[20%]' placeholder='Search' onChange={handleSearchLanguageTo}></input>
                        <div className='overflow-y-scroll h-[80%]'>
                            {
                                languageToOptions.map((language, index) => (
                                    <button key={index} value={language} onClick={ (e) => handleLanguageTo(e,language) } className='w-full h-[15%] md:h-[33%]'>{`${language.language}/${language.nativeWriting}`}</button>
                                ))
                            }
                        </div>  
                    </div>
